...
import { MONTHS, WEEKDAYS } from "@/lib/content";
...
export function Calendar({ selected, onChange }: Props) {
  ...
  return (
    <div className="w-full rounded-3xl bg-white sticker sticker-outline p-4">
      <div className="mb-3 flex items-center justify-between">
        <button
          type="button"
          onClick={() => canGoPrev && shift(-1)}
          disabled={!canGoPrev}
          className="tap-target rounded-full px-3 text-2xl text-ink disabled:opacity-25"
        ...
        <button
          type="button"
          onClick={() => shift(1)}
          className="tap-target rounded-full px-3 text-2xl text-ink"
        ...
        {cells.map((day, i) => {
          ...
          return (
            <motion.button
              ...
              animate={{
                backgroundColor: isSelected ? "var(--pink)" : "rgba(0,0,0,0.04)",
                color: isSelected ? "#3b2a2a" : "#6b5550",
